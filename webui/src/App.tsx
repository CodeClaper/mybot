...
export default function App() {
      ...
      (async () => {
        ...
        try {
          ...
          setState({
            ...
            refresh_token: boot.refresh_token,
            model_name: boot.model_name ?? null
          });
        } catch (e) {
          if (cancelled) return;
  ...
  const bootstrapWithPassword = useCallback(
    (username: string, password: string) => {
      let cancelled = false;
      (async () => {
        setState({ status: "loading" });
        try {
          const boot = await fetchLogin("", username, password);
          if (cancelled) return;
  ...
  useEffect(() => {
    ...
    return bootstrapWithSecret(access_token, refresh_token);
  }, [bootstrapWithSecret]);
  ...
  if (state.status === "auth") {
    return (
      <LoginForm
        failed={!!state.failed}
        onLogin={(username, password) => bootstrapWithPassword(username, password)}
      />
    );
  }
