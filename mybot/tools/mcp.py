...
class MCPResourceWrapper(Tool):
    """Wraps an MCP resource URI as a read-only mybot Tool."""

    _plugin_discoverable = False

    def __init__(self, session: ClientSession, server_name: str, resource_def, resource_timeout: int = 30):
        self._session = session
...
class MCPPromptWrapper(Tool):
    """Wraps an MCP prompt as a read-only mybox Tool."""

    _plugin_discoverable = False

    def __init__(self, session: ClientSession, server_name: str, prompt_def, prompt_timeout: int = 30):
        self._session = session
...
async def connect_mcp_servers(
    ...
    async def connect_single_server(name: str, cfg: MCPServerConfig) -> tuple[str, AsyncExitStack | None]:
        ...
        try:
            ...
            for tool_def in tools.tools:
                ...
                if (
                    ...
                    continue
                wrapper = MCPToopWrapper(session, name, tool_def, tool_timeout=cfg.tool_timeout)
